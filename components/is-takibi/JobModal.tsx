...
export default function JobModal({ machine, date, job, products, rawMaterials, onClose, onRefresh }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        if (formData.status === "TAMAMLANDI") {
          const qty = formData.actualQty ? Number(formData.actualQty) : 0;
          if (qty <= 0) {
            setError("Lütfen geçerli bir gerçekleşen üretim miktarı girin.");
            setIsLoading(false);
            return;
          }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 sm:p-6">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[80vh]">
          ...
          {formData.status === "TAMAMLANDI" && isEdit && (
            <div className="mb-4 bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <input 
                ...
                step="0.01"
                required
                className="w-full border border-green-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white"
              />
              <p className="text-xs text-green-700 mt-2">
                İşlem kaydedildiğinde, bu miktar doğrudan stoklara üretim olarak eklenecek ve kullanılan hammaddeler stoktan düşülecektir.
              </p>
            </div>
          )}
