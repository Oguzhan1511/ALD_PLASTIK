...
export default function IsTakibiClient({ machines, products, rawMaterials, initialDate, initialSchedules }: any) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (activeNotification) return; // Zaten bildirim varsa bekle
  ...
  }, [schedules, activeNotification, dismissedNotifications]);

  const handleNotificationAction = async (accept: boolean) => {
    if (!activeNotification) return;
    const { job, type } = activeNotification;
    
    // UI'ı hemen kapat ve bu işi bir daha göstermemek için listeye al
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-10">
      ...
      {/* Uyarı Modalı (Toast tarzı) */}
      {activeNotification && (
        <div className="fixed bottom-6 right-6 z-50 animate-in slide-in-from-bottom-5 fade-in duration-300">
          <div className="bg-white border-l-4 border-blue-500 rounded-lg shadow-2xl p-5 max-w-sm w-full">
            <div className="flex items-start">
              ...
              <div className="ml-3 w-0 flex-1">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  <strong className="text-gray-800">{activeNotification.job.machine?.name}</strong> için planlanan 
                  <strong className="text-gray-800"> {activeNotification.job.product?.name}</strong> üretiminin 
                  {activeNotification.type === 'START' ? ' başlama' : ' bitiş'} zamanı geldi.
                </p>
                <p className="mt-2 text-sm text-gray-600 font-medium">
                  Durumu <span className="text-blue-600">'{activeNotification.type === 'START' ? 'Devam Ediyor' : 'Tamamlandı'}'</span> olarak güncelleyelim mi?
                </p>
              </div>
            </div>
